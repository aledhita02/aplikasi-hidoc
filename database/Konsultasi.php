...
class Konsultasi
{
  ...
  public function hitungKonsultasiProcess($id)
  {
    $sql = "SELECT * FROM tkonsultasi WHERE id_pasien=? AND status_konsul='process' ";
    $stmt = $this->pdo->prepare($sql);
    $stmt->execute([$id]);
    $count = $stmt->rowCount();
    return $count;
  ...
  public function cariChat($id, $keyword)
  {
    $sql = "SELECT * FROM tkonsultasi,tuser WHERE id_pasien='$id' AND status_konsul='process' AND tkonsultasi.id_dokter = tuser.id_user AND tuser.nama_user LIKE '%$keyword%'";
    $stmt = $this->pdo->query($sql);
    return $stmt;
  }
  public function cariChatHistory($id, $keyword)
  {
    $sql = "SELECT * FROM tkonsultasi,tuser WHERE id_pasien='$id' AND status_konsul='finish' AND tkonsultasi.id_dokter = tuser.id_user AND tuser.nama_user LIKE '%$keyword%'";
    $stmt = $this->pdo->query($sql);
    return $stmt;
  }

  public function cariChatDokter($id, $keyword)
  {
    $sql = "SELECT * FROM tkonsultasi,tuser WHERE id_dokter='$id'AND status_konsul='process' AND tkonsultasi.id_pasien = tuser.id_user AND tuser.nama_user LIKE '%$keyword%'";
    $stmt = $this->pdo->query($sql);
    return $stmt;
  }

  public function cariChatRiwayatDokter($id, $keyword)
  {
    $sql = "SELECT * FROM tkonsultasi,tuser WHERE id_dokter='$id'AND status_konsul='finish' AND tkonsultasi.id_pasien = tuser.id_user AND tuser.nama_user LIKE '%$keyword%'";
    $stmt = $this->pdo->query($sql);
    return $stmt;
  }
