...
if(isset($_SESSION['role'])){
  if(isset($allowedPages[$_SESSION['role']]) && in_array($page, $allowedPages[$_SESSION['role']]) && isSafePage($page) ){
      ...
      header("Location: http://localhost:80/portofolio/aplikasi-hidoc/index.php?p=" . $allowedPages[$_SESSION['role']]['default_page']);
      exit();
    }
  ...
  if (in_array($page, $allowedPagesGuest['default_guest']) && isSafePage($page)) {
    ...
    header("Location: http://localhost:80/portofolio/aplikasi-hidoc/index.php");
    exit();
  }
}
